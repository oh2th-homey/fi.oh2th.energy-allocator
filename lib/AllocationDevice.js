...
class AllocationDevice extends Homey.Device {
  ...
  async onInit() {
    ...
    this.log(`initialised (monitoredDeviceId=${this.getMonitoredDeviceId() || 'none'}`
      + `${this.getMonitoredDeviceId() ? ` cap=${this.getMonitoredCapability()}` : ''})`);
  }
  ...
  /**
   * Which `meter_power(.x)` capability on the monitored device this allocator
   * follows. Only meaningful when {@link getMonitoredDeviceId} is non-null.
   * @returns {string}
   */
